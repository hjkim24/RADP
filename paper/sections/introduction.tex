...
\begin{itemize}
  ...
  \item An ablation that turns on (i) outer-subset enumeration so
        the DP can auto-prune pathologically-slow workers and
        (ii) a per-hop overhead term $\gamma_{\mathrm{hop}}$ in
        $T_{\mathrm{comm}}$, showing the L $\succ$ T gap at $C \geq
        4$ \emph{widens} rather than closes
        (\S\ref{sec:eval:cost-model}). The remaining gap is a
        multi-stream system-level effect, not a missing
        wire-cost term, bounding what cost-model refinements
        alone can buy back for throughput placement.
\end{itemize}
