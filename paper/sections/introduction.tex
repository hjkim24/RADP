...
\IEEEPARstart{D}{istributed} LLM inference on edge clusters has moved
from research curiosity to viable deployment.  Petals~\cite{borzunov2023petals,borzunov2023distributed}
...
that the edge regime breaks.  First, \emph{homogeneity}: edge
fleets routinely mix GPU tiers (AGX Orin and Orin Nano) and
sometimes CPU-only boards.  On our 6-worker Jetson fleet (plus
AGX Xavier coordinator), AGX MAXN is $1.36\times$ faster than
Nano CUDA at OPT-350M seq=64 (launch-bound regime; larger models
would expose more), and including Nano CPU boards extends the
within-fleet gap to $\sim$76$\times$
(\S\ref{sec:eval:setup}).  Second,
\emph{reliability}: boards crash, internal storage fills up, networks
...
on the cluster --- a comfortable assumption on a memory-abundant
consumer GPU but unaffordable on a 4\,GB Nano.  Petals' greedy placement leaks
...
\begin{itemize}
  ...
  \item Live measurement on a 6-worker Jetson fleet (with an AGX
        Xavier coordinator) across
