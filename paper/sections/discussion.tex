...
recomputation, sum-of-stages replication, or a reactive reconfiguration that
...
simultaneous stage failures from the same stored bytes with recovery
latency comparable to a single failure when their backups reside on
distinct nodes.  In an offline placement analysis
...
from a request loss or a reactive reconfiguration into a delay of two to
four decode steps over the measured range ($P\le32$).
