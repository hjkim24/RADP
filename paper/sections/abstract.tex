...
language models (LLMs) as pipelines of decoder layers over heterogeneous
edge devices.  A stage failure then loses the request's key-value (KV)
...
and reload the pipeline.  This paper presents KV-CARE (KV Cache Availability
and Recovery at the Edge).  Cross-stage parity folds the protected stages'
KV columns into one parity column (two for simultaneous failures) and
reconstructs a failed stage's KV cache bit-exactly without recomputing the
...
single failure when their backups reside on distinct nodes.  In an offline
