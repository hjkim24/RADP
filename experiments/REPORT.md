...
**근데 "무해"는 아니다 — margin 분석 (안전계수 ≈ 1×).** 같은 스크린에서 스텝별 top-2 logit을 기록해 "얼마나 flip에 가까웠나"를 정량화했다(greedy 256스텝, 매 스텝 top-2 logit 기록). **decision margin(top1−top2 logit)**: min **0.031** / median 4.88 / max 9.8 — 대부분 결정은 압도적으로 확실하지만 최소 margin은 0.031(near-tie). **cross-tier logit 드리프트(승자 토큰)**: max **0.031** / median 0.0(대부분 비트 동일). 즉 **최소 margin과 최대 드리프트가 같은 크기(둘 다 ≈ 2⁻⁵)**, 안전계수 min(margin)/max(drift) ≈ **1×**. 256토큰이 안 뒤집힌 건 near-tie 스텝(margin 0.03)과 non-zero-drift 스텝이 **우연히 안 겹쳐서**지 구조적 안전이 아니다 — 다른 프롬프트/더 긴 생성이면 겹쳐서 flip이 날 수 있다. 그러니 정확한 문장은 "출력에 무해"가 아니라 **"재계산 복구는 이종 티어에서 출력 뒤집힘과 종이 한 장 차이(드물지만 실재하는 리스크), store-KV는 그 리스크가 0"**. 코드: `experiments/probe_output_divergence.py`. 미측정: 실제 flip을 내는 프롬프트/길이(안전계수 1×라 탐색 가치 있음), sampling(RNG cross-device 교란이 별개로 섞임), 더 깊은 stage.
