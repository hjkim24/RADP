...
**일반 Reed-Solomon(k≥3) 확장은 검토 후 미구현 — 이 fleet에선 replicate에 지배당함.** RAID-6는 k=2 Reed-Solomon 소거부호이고, k개 패리티 blob(P,Q,R,…)을 두면 동시 k개까지 복구하도록 일반화된다(Cauchy 생성행렬 + k×k GF 역산; 순진한 Vandermonde는 k≥3에서 일부 소거 패턴에 특이). 하지만 저장 geometry가 확장의 값을 먼저 부정한다: k-parity 저장 = k×max(non-head), replicate = Σ(non-head)이므로 **k-parity가 replicate를 이기려면 `k < Σ(non-head)/max(non-head)`** (balanced 파이프라인이면 = non-head 개수 m). 이 fleet은 Σ/max=**2.25**라 k=1,2만 replicate 미만이고, **k=3부터는 저장이 replicate 초과(1.33×)인데 복구 내성은 3개로 replicate의 "임의 개수"보다 약함 → 완전 열등(dominated).** 즉 일반 RS는 **non-head stage가 많고 균형 잡힌(Σ/max 큰) 큰 파이프라인**에서, f<Σ/max 범위의 중간 다중 내성이 필요할 때만 값을 한다 — 현 5-stage head-heavy fleet의 실질 상한은 RAID-6(k=2). k≥3은 그 레짐이 생기면 착수(future work). GF(2⁸) 필드 산술(`gf256.py`)은 이미 재사용 가능하므로 확장 시 추가되는 건 Cauchy 계수 + 일반 m×m 역산뿐.
