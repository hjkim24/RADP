...
### REPORT / paper follow-ups (from the final review — track when writing REPORT.md/Task 8, do NOT rationalize)
- **Fault model wording.** The in-process injection is NOT a SIGKILL — it is a controlled monkeypatched mid-forward `raise`, deliberately timed to land *after* the position's mirror push (the injector polls `get_history` before raising). That ordering is what steers execution into the surgical branch. On a real SIGKILL the branch taken depends on whether the fire-and-forget mirror RPC won the race with the kill. REPORT/paper must describe the demonstrated case as a **compute-time crash** (OOM/thermal/CUDA error mid-forward), not "SIGKILL". (Fix the stale "SIGKILL" wording in `experiments/b1_ft_baselines.py:1` docstring at that time.)
- **TTR is two different measures.** In-place lines (surgical, full-replay) report `ttr_seconds` = single recovery-step wall; cold-restart reports `ttr_seconds` = `total − reference_wall` (whole extra wall). The headline "surgical ≪ full-replay ≈ cold-restart" compares a per-step latency against a whole-run delta for the "≈". Spec-sanctioned TTR taxonomy, but the REPORT table must **label the two TTR columns as different measures** so the "≈" isn't misread as same-unit.
- **Surgical safety is load-bearing on `get_history` returning a contiguous prefix that stops at the first gap** (`activation_cache.py`): a lagging/gappy async mirror shortens `history` → `len(history) <= P` → fallback to full-replay → never a wrong token. Keep this invariant if `ActivationCache` is refactored.
