...
| Letter scope residuals | R0, R1, R2, R3, DA | This section lists Fig. 1; the stale "spread" row above is corrected; J8 (units beyond the table note) and J10 (RAID naming) are recorded as not applied; J10's exactness novelty clause was applied (§II-F after the new §II-B) and is now listed. |
...
Post-decision additions (2026-09-07, page budget lifted; 11 pages): §IV-A "Software environment" (JetPack/L4T, CUDA/cuDNN, Python, PyTorch and transformers versions, power modes, wired LAN — item 20/K20, R1 and R3); §IV-C "Coordinator sizing" with the per-request formula, the as-built 7B numbers and the concurrent-request bound, plus a Limitations sentence on coordinator failure and a standby (item 17, R3; DA coordinator SPOF); §II-B "Fault Tolerance in Edge and IoT Systems" positioning KV-CARE against five IEEE IoT-J papers on edge-service fault tolerance and coupled placement (R0 fit item; S10). Reference count 34 → 39, IoT-J entries 1 → 7 (CASIT added to §I; one TII battery-management reference dropped as no longer cited). The introduction's opening was reframed from industrial-first to on-site IoT deployments broadly, the abstract's results sentence was split into four, about twenty semicolon-chained sentences were split with no change of number or qualifier, and the small-model microscopy paragraph was restructured.

---

# Round 5 — fourth verification re-review (2026-09-08)

Panel: R0, R1, R2, R3, DA all Accept; nothing blocks. All five Round-4 MUST errata verified applied (each reviewer grepped the erratum strings across every section; the earlier propagation pattern did not recur). Journal fit moved 6 → 7 for the first time in five rounds on the strength of §II-B and the reframed introduction. Residuals were confined to the new text and are applied:

| Item | Reviewers | Change |
|---|---|---|
| "about 30 GB free" on the coordinator has no saved-profile source; the prototype caps its parity, mirror and replica stores at 256 MB with whole-request eviction; the mirror holds six vectors per position (the head's is primed), not five; "about 30" was 33.7 | R1, R2, R3, DA | §IV-C rewritten as a per-byte comparison: 224/448 MB + 96 MB of mirror (six 8 kB vectors) vs 832 MB + 96 MB; single parity holds about 2.9× as many requests per byte of coordinator memory; the 256 MB store caps and whole-request eviction stated; no absolute request count claimed. $N_{\mathrm{ctx}}$ defined. |
| "JetPack 6.1" for all workers, while ao-1 runs L4T R36.4.3 (JetPack 6.2) and MAXN SUPER implies 6.2 on the Nanos; "ao-2 as flashed" has no source | R0-N18, R1-NEW-20, R3, DA | "JetPack 6 (L4T R36.4.x, …)"; the ao-2 power-mode clause removed. LAN rate remains unprinted (no saved profile records it). |
| §II-B: Sun's framework is proactive; Zhang (joint) uses a two-step method rather than "showing" joint solution is necessary; the delta's "substitute chosen after placement" fits Ma least | R0, R2, DA | "proactively migrate tasks away from nodes predicted to fail"; "treat the two coupled decisions in one procedure rather than in sequence"; "where a substitute node is chosen it is chosen after…". |
| Introduction: CASIT does not state on-site deployment; "assistants in homes and buildings" uncited | DA | "LLM agents coordinating IoT devices"; the uncited clause removed. |
| Letter: the tolerance sentence's "fetches the surviving columns at recovery" clause was not in the file (trimmed for pages before the budget was lifted); "eight-device bound (unverified)" is settled by scheduler.py (max_search_devices=8); IoT-J count; stale §II-E | R0-N20, R1, R2, R3, DA | The clause is now in §IV-C (page budget lifted); §III-E states the eight-device bound (109 592 orderings); counts and section letters corrected above. |
...
Advisor final pass (2026-09-13): abstract trimmed to 244 words; Recovery-State Footprint redefined as the bytes a family's primary recovery path requires (the common per-position input mirror excluded), which keeps the 3.7× figure and reconciles §IV-A with Table I; Table I's Reconfigure row now carries dashes with a footnote giving the two measured latencies in seconds; binary units printed as KiB/MiB/GiB (device SKUs stay in GB); §III-E describes the greedy backup solver (device order, host filter on free minus own stage minus reservations, minimum estimated transfer plus recompute time, first-candidate tie-break, running reservations); the two-to-four-decode-step claims are scoped to the measured range P ≤ 32; "reconfigure in minutes" softened to reactive reconfiguration and reload, with the cached-profile variant named as unmeasured; §III-B states the tail-stage replay fallback; the two-failure claim is stated as recovery latency; American spelling; "Small-model diagnostic study".
