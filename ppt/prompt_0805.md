...
- **그림 3종은 덱 끝의 "STASH" 슬라이드에 이미 얹혀 있다.** 파일에서 다시 불러오지 말고 그
...
- 시각물: **STASH의 fig_recovery_2d** — 5계열 2D Pareto(저장 × P=32 복구시간, log-X). KV-RAID가 좌하단 코너, DejaVu는 TTR 동률·저장 2.25×
- 이번 주 질문 한 줄: **KV-RAID는 단일 실패(RAID-5)만 복구됨. 동시 2개가 죽으면?**
- 캡션: OPT-350M 5-stage 이종 체인 / victim `on-1`
