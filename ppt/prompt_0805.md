...
**소주제는 2단 계층으로.** 한 슬라이드에 소주제가 여럿이면(예: 작동 슬라이드의 기존 한계·작동·
선행·워커 무변경) **모든 줄을 같은 `•` 로 나열하지 마라** — 그러면 소주제 제목이 내용 불릿과
안 구분된다. 아래 인테이크에서 **`▸` 로 시작하는 줄 = 소주제 제목**이니 **불릿 없이 굵게**(강조
파랑 `#0070C0` 가능, 위에 한 줄 여백) 쓰고, 그 아래 **`-` 로 시작하는 줄 = 그 소주제의 `•`
불릿**로 한 단 들여써라. `a.`·`b.`·`b2.` 같은 글자는 슬라이드에 찍지 마라 — 개념 이름만.
...
▸ 기존 한계
- KV-RAID-5는 XOR blob P가 1개 → 방정식 1개로 미지수 2개를 못 풂
- 2개 죽으면 Petals로 폴백(비쌈)

▸ 작동
- 두 번째 blob **Q(GF(2⁸) 가중합)** 추가 → 2개 죽으면 P·Q로 **2×2 GF 연립** 풀어 둘 다 재계산 0으로 복원
- 토글 `RADP_PARITY_K=1|2` — **k=1은 KV-RAID-5 그대로**, k=2도 1개만 죽으면 P만 써서 RAID-5 포함

▸ 선행
- **RAID-6 = k=2 Reed-Solomon 소거부호**(Anvin)
- KV를 저장해 복구하는 데이터센터 방식(DejaVu 복제 / GhostServe erasure-coding)을 이종 엣지 다중 실패로 옮긴 것 — GhostServe가 stage 간 parity를 다음 과제로 남긴 자리

▸ 워커 무변경
- Q는 워커가 원래 보내던 KV 조각으로 coordinator가 알아서 계산
...
▸ 장단점
- 저장 = KV-RAID-5 **16384** / KV-RAID-6 **32768** / DejaVu **36864** B/토큰
- 1개만 죽으면 여전히 P만 쓰니 KV-RAID-5랑 비용 같음. 워커 코드는 안 건드림

▸ 어디까지 되나 (딱 2개까지)
- blob을 k개 두면 k개까지 복구됨. DejaVu보다 저장이 적으려면 `k < Σ/max` (고르게 나눈 파이프라인이면 non-head stage 수)
- 우리 fleet은 `Σ/max=2.25` → k=1, 2만 DejaVu보다 적음. **k=3부터는 저장이 DejaVu보다 많고(1.33배) 내성도 약함 → 그냥 DejaVu가 나음.** 일반 RS는 stage 많고 고른 큰 파이프라인에서나 의미(다음 과제)

**3. KV-RAID-6 — 라이브 실측 (이번 주 핵심)**
- **소제목(주장) = 이번 주 헤드라인:** 동시 2-실패 라이브 복구 5/5 bit-correct, 복구시간 실패위치 무관
▸ 결과 (5/5 bit-correct)
- 라이브 fleet에서 `on-1`·`on-6`을 동시에 죽임. 5개 위치(P=4·8·16·24·32) 전부에서 **복구 출력 = 장애 없을 때 출력**. GF로 2개 되살리는 경로가 실제로 돌았는지 로그로도 확인
- `TTR(P) = 30.29 s + 2.78 ms·P` → **기울기 ≈ 0** (실패 위치가 깊어져도 복구시간이 안 늘어남 = 재계산 안 함). 단일 KV-RAID(0.87)·DejaVu(2.67)와 같은 평평한 그룹, Petals(16.2)·Recompute(164)는 위치 따라 늘어남
...
▸ 꼭 짚을 점 — 고정 부분 30.3초는 알고리즘 비용이 아님 (슬라이드에 반드시)
- 이 fleet은 백업이 전부 `on-2` 한 노드에 몰림 → 2개 죽으면 약한 Nano `on-2` 하나가 stage 3개를 떠안고 가중치를 새로 불러옴(cold load). 1개만 죽었을 땐 KV-RAID 복구가 284 ms
- 이 몰림은 고정 지연 30초만 더할 뿐 **기울기는 안 건드림** → "재계산 0(기울기 ≈ 0)" 결론은 그대로 유효. 백업을 여러 노드로 나눠 두면 절대 시간도 내려감(다음 과제)

▸ 라이브에서 잡은 버그
- `on-1`이 head 바로 뒤라, 죽으면 시스템이 head가 죽은 걸로 잘못 지목 → 2개 복구로 가기 전에 head 처리 로직이 먼저 걸려 폴백. 죽은 노드 목록을 먼저 보도록 순서를 바꿔 고침, 이후 5/5 정상
