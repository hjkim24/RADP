...
## 작업 4 — `장애 깊이` 표현 교체 (그림 재삽입 포함)

`깊이` 가 파이프라인 stage 순서로 읽혀서 victim 위치와 헷갈린다. P는 **죽기까지 만든
토큰 수**다. 아래 문구를 찾아 바꾼다 (해당 슬라이드의 다른 요소는 건드리지 마라):

| 찾을 것 | 바꿀 것 |
|---|---|
| `복구 시간이 장애 깊이를 안 탐` | `복구 시간이 장애 시점을 안 탐` |
| `기울기 = 깊이 P당 추가 비용` 또는 `기울기가 곧 "깊이 P 당 추가 비용"` | `P = 죽기까지 만든 토큰 수. 기울기 = P 1개당 추가 비용` |
| 표의 `깊이 P당 비용` 행 이름 | `P 1개당 추가 비용` |
| `깊이 비용을 정상 운영 중 …` | `장애 시점 비용을 정상 운영 중 …` |
| `parity 기울기는 장애 깊이·victim 위치 …` | `parity 기울기는 장애 시점·victim 위치 …` |
| `비용이 장애 깊이 P에 비례` | `늦게 죽을수록 비쌈 (P에 비례)` |

TTR 그래프 슬라이드의 그림도 축 라벨이 바뀌었으므로 **다시 얹는다**:
`paper/figures/fig_recovery_ttr_slide.png` — 6.42 in, 현재 자리 그대로, 배율 100%.
